...
face_detector = FaceDetector(
    weight_path=home + "model/Resnet50_Final.pth", device=conf.device
)
...
def faces_preprocessing(faces):
    ...
    mu = torch.as_tensor([0.5, 0.5, 0.5], dtype=faces.dtype, device=conf.device)
...
def main():
    ...
    clf = TorchRandomForestClassifier(
        nb_trees=100, nb_samples=3, max_depth=5, bootstrap=True
    )
    clf.fit(faces, names)
